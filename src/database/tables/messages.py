...
class Comments(Table):
    ...
    async def create_comment(self, comment: Comment) -> None:
        await self.db.execute(
            ...
            comment.id, comment.username, comment.comment, comment.comment_id
        )

    async def delete_comment(self, id_: str, username: str, comment_id: str) -> None:
        await self.db.execute("DELETE FROM comments WHERE id = $1 AND username = $2 AND comment_id = $3", id_, username, comment_id)
    ...
    async def update_comment(self, id_: str, username: str, comment: str, comment_id: str) -> None:
        await self.db.execute("UPDATE comments SET comment = $1 WHERE id = $2 AND username = $3 AND comment_id = $4", comment, id_, username, comment_id)
