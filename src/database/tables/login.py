...
from src.utils.models import User as UserModel
...
class User(Table):
    ...
    async def add_bookmark(self, username: str, post_id: int) -> None:
        await self.db.execute(
            "UPDATE users SET bookmarks = array_append(bookmarks, $1) WHERE username = $2",
            post_id,
            username,
        )
    ...
    async def remove_bookmark(self, username: str, post_id: int) -> None:
        await self.db.execute(
            "UPDATE users SET bookmarks = array_remove(bookmarks, $1) WHERE username = $2",
            post_id,
            username,
        )
